...
if ($model->attend_id == 1) {
    $default_departure      = date('2016-10-09');
}elseif ($model->attend_id == 2 || $model->attend_id == 5) {
    $default_departure      = date('2016-10-12');
}elseif ($model->attend_id == 3) {
    $default_departure      = date('2016-10-10');
}elseif ($model->attend_id == 4 || $model->attend_id == 6) {
    $default_departure      = date('2016-10-11');
}

$default_departure_end      = date('2016-10-13');

if ($model->attend_id == 6) {
    $default_departure_end      = date('2016-10-12');
}
...
$code_participant       = Participant::find()->where(["user_id"=>Yii::$app->user->identity->id])->one();

if ($status_companion) {
    $is_companion = FALSE;
}else {
    if ($code_participant->variety_id == 10 || $code_participant->variety_id == 13 || $code_participant->variety_id == 14 || $code_participant->variety_id == 16 || $code_participant->variety_id == 17 || $code_participant->variety_id == 20 || $code_participant->variety_id == 21 || $code_participant->variety_id == 22 || $code_participant->variety_id == 24 || $code_participant->variety_id == 26 || $code_participant->variety_id == 30) {
        $is_companion = TRUE;
    }else{
        $is_companion = FALSE;
    }
...
<div class="participant-form">
    ...
    <div class="row s">
        <div class="col-md-12 col-sm-12">
            ...
            <div class="jumbotron" id="formulirrepresentative" style="display:none">
                <h4 class="pull-left" style="font-weight:500px; font-size:24px; margin:0">Representative</h4>
                <hr class="style-one">
                
                “Dear <b><?= $titles;?>, <?= $model->full_name;?></b> will you be attending WCF 2016 by yourself ?”<br><br>

                <div style="margin-top:5px;">
                    <div class="col-xs-6">
                        <?= Html::Button('Yes, I will attend WCF 2016 by myself.', ['class' => 'btn btn-success btn-block', 'onclick' => 'ActionYes()', 'style' => 'color:white !important;']) ?>
                    </div>
                    <div class="col-xs-6">
                        <?= Html::Button('No, I am a representative of '.$titles.', '.$model->full_name.'', ['onclick' => 'ActionNo()', 'class' => 'btn btn-danger btn-block', 'style' => 'color:white !important;']) ?>
                    </div>
                </div>&nbsp;
            </div>
            ...
            <div id="hiddenform" style="display:none">
                ...
                <div class="jumbotron">
                    ...
                    <hr class="style-one">
                        ...
                        <?= $form->field($data_representative, 'assignment_letter')->widget(FileInput::classname(), ['options' => ['placeholder'=>'Please upload your Assignment Letter/Surat Tugas', 'accept' => '*']])->label('Assignment Letter'); ?>

                        <div class="form-group">
                            <?= Html::submitButton($data_representative->isNewRecord ? 'Submit' : 'Submit', ['name' => 'submit-representative-form','class' => $data_representative->isNewRecord ? 'btn btn-success col-md-12' : 'btn btn-primary col-md-12', 'style'=> 'color:white !important;','onclick' => "return confirm('Thanks for informing us about your representation, we will inform you soon after reviewing your Assignment Letter/Surat Tugas.')",]) ?>
                        </div>&nbsp;
                        <br><em><span style="font-size: 14px;" align="justify">* Make sure about this, because you can't undo this action.</span></em>
            ...
            <div id="hiddencompanion" style="display:none">

                <div class="jumbotron">
                    <h4 class="pull-left" style="font-weight:500px; font-size:24px; margin:0">Companion</h4>
                    <hr class="style-one">

                    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]) ?>

                    <?= $form->field($model_companion, 'full_name')->widget(MultipleInput::className(), [
                        'limit' => 5,
                        'columns' => [
                            [
                                'name'  => 'title',
                    ...
                    <div style="margin-top:5px;">
                        <div class="col-xs-6">
                            <?= Html::Button('Cancel', ['onclick' => 'ActionCancel()', 'class' => 'btn btn-danger btn-block', 'style' => 'color:white !important;']) ?>
                        </div>
                        <div class="col-xs-6">
                            <?= Html::submitButton('Submit', ['name' => 'submit-companion-form','class' => 'btn btn-success btn-block', 'style'=> 'color:white !important;']) ?>
                        </div>
                    </div><br><br>
                    <em><span style="font-size: 14px;" align="justify">* You may suggest guest companion with maximum 5 (five) persons. Click (+) button to add companion candidates. Please note that companion candidates will be moderated by the committee, so you should add them based on the priority (number one is the first priority). WCF 2016 will provide accommodation for 1 (one) person of guest companion, the rest accepted companions have to responsible for their own expenses (airfare ticket and accommodation ) during the event.</span></em>
                    <?php ActiveForm::end(); ?>
                </div>
            ...
            <div class="jumbotron">
                <div style="padding:5px;">
                    ...
                    <!-- Button Option -->
                    <h4 style="font-weight:500px; font-size:24px; margin:0;">Personal Details 
                        <?php if (empty($status_representative) && ($model->variety_id !== 25 || $model->variety_id !== 28) || $model->variety_id !== 19 || $model->variety_id !== 35) { ;?>
                            <?= Html::Button('Representative', ['onclick' => 'ActionRepresentative()', 'class' => 'btn btn-info pull-right', 'style' => 'color:black !important;']) ?>
                        <?php } ;?>
                        <?php if ($is_companion) { ;?>
                            <?= Html::Button('Guest Companion', ['onclick' => 'ActionCompanion()', 'class' => 'btn btn-info pull-right', 'style' => 'color:black !important;margin-right:10px !important;']) ?>
                        <?php } ?>
                    </h4>
                    <!-- Button Option -->
                ...
                <?php } ?>
                                        ...
                                        <?= $form->field($model, 'date_departure')->widget(DatePicker::classname(),[
                                           ...
                                           'options' => [
                                            ...
                                            'pluginOptions' => [
                                                ...
                                                'format' => 'yyyy-mm-dd',
                                                'todayHighlight' => true,
                                                'startDate' => $default_departure_end,
                                                'endDate' => date('2016-10-15'),
                                            ]
                    ...
                    <div class="jumbotron">
                        ...
                        <div class="row">
                            ...
                            <?php if($model->variety_id == 19 || $model->variety_id == 35){ ?>


                                <div class="col-md-4 col-sm-6">
                                     <?= $form->field($model, 'attend_id')->widget(Select2::classname(), [
                                        'data' => ArrayHelper::map(Attend::find()->where(['id'   => 1])->all(), 'id', 'information'),
                                        /*'pluginOptions' => [
                                            'allowClear' => true
                                        ],*/
                                    ])->label("Attend At"); ?>
...
/*function Date_arrival_changed(e){
    document.getElementById('date_arrival').innerHTML = document.getElementById('participantregistration-date_arrival').value;
}
